...
```bash
cd server && ./gradlew clean bootJar          # 产物 server/build/libs/aiwatch-server-1.0.16.jar
cd ../agent && VERSION=1.0.16 bash build-dist.sh   # 产物 agent/dist/install/
```

把 `aiwatch-server-1.0.16.jar` 和 `agent/dist/install/` 拷到生产机仓库对应位置。
...
docker run --rm -e VERSION=1.0.16 -e GOPROXY=https://goproxy.cn,direct \
