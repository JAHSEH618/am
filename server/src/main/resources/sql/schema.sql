...
(
    ...
    team_grade_dist_json          JSON          DEFAULT NULL COMMENT '等级分布 {"S":1,"A":3,...}',
    team_narrative_json           JSON          DEFAULT NULL COMMENT 'LLM 团队总评 {overview,highlights,risks,recommendations}',
...
(
    ...
    composite_grade             VARCHAR(2)    DEFAULT NULL COMMENT 'S/A/B/C/D（v2 公式）',
    composite_confidence        VARCHAR(8)    DEFAULT NULL COMMENT 'normal / low（10-19 会话）',
    composite_breakdown_json    JSON          DEFAULT NULL COMMENT 'v2 五维子分+权重+收缩参数',
    -- 标记
    ...
    narrative_json              JSON          DEFAULT NULL COMMENT 'LLM 个人评语 {level_summary,evidence,strengths,weaknesses,suggestions}',
    retry_count                 INT           DEFAULT NULL COMMENT '窗口内重试/卡壳次数（daily_summary 求和）',
    retry_per_active_hour       DECIMAL(8,4)  DEFAULT NULL,
    tool_call_count             INT           DEFAULT NULL COMMENT '窗口内工具调用次数（daily_summary 求和）',
    created_time                DATETIME      NOT NULL,
    PRIMARY KEY (id),
