...
# 客户端
cd agent && go test ./...
VERSION=1.0.19 bash build-dist.sh   # 交叉编译四平台 → dist/install/
```
...
│   ├── build.gradle      artifact = aiwatch-server，version 1.0.19
│   └── src/main/
...
当前发布版本 **1.0.19**。本仓库由原 *ai-work-platform*（在线工时与成本核算）重定位为 **AIWatch**，去成本视角、聚焦 AI 使用观测；产品愿景见 [`docs/design/aiwatch-design-v2.0.md`](docs/design/aiwatch-design-v2.0.md)。包名 `com.am.server` 中的 `am` = *AI Monitoring*，非公司名。
